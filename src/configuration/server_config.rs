        ...
        Some(table) => {
            for (key, value) in table {
                match key.as_str() {
                    ...
                    "zk_connect_string" => match value.as_str() {
                        Some(val) => config.zk_connect_string = String::from(val),
                        None => warn!("Couldn't parse zk_connect_string value {} as an integer", value)
                    },
